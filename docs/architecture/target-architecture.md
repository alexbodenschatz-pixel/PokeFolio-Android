...
One process-scoped Android cloud service owns that session for the account bridge and later catalog/sync bridges. Activity recreation reuses this single session gate, preventing two host instances from racing the same rotating refresh credential. The packaged WebView receives only token-free account/device state through a bounded serial callback facade; it cannot select a user ID or read credentials. A configured unauthenticated host attempts one persistent-session restore during page startup. Activity shutdown drops its pending callbacks without closing the process-owned session. Visible account controls and cloud-collection wiring remain separate product steps.
