...
- logout revokes the current token family; a selected foreign session or “logout other devices” revokes only owned, non-current families, while the current session requires explicit logout;
