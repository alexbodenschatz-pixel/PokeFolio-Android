...
Der gemeinsame `collection-cloud-driver.js` lädt gezogene Holdings über `PokeCatalog`, materialisiert sie auch auf einem zweiten Gerät und verknüpft bestehende Rich-Metadaten ohne kontofremde Wiederverwendung. Bereits verknüpfte lokale Mengenänderungen werden als atomare Deltas, Notizen, Sprache, Variante und Zustand als optimistic-concurrency-Updates vorgemerkt. Bei laufender Queue, unvollständigem Pull, beschädigtem Holding oder Kontowechsel wird kein veralteter Snapshot geschrieben. Noch offen ist der asynchrone Outbox-Pfad, der eine vollständig neue, erst nach der Migration gescannte Karte katalogisiert und als neues Holding anlegt.
