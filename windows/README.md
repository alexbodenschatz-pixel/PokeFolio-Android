...
`PokeFolioApiClient` kapselt Registrierung, Login, authentifizierten Passwortwechsel, Sitzungswiederherstellung, Refresh-Rotation, Logout, Geräteauflistung/-widerruf, globale Kartenauflösung/-abfrage sowie rohe Sync-Push-/Pull-Aufrufe. Produktive Ziele müssen eine pfadfreie HTTPS-Origin verwenden; unverschlüsseltes HTTP ist ausschließlich für Loopback-Entwicklung erlaubt. Redirects und Cookies sind deaktiviert, Antworten sind größenbegrenzt und ein abgewiesener Zugriff wird höchstens einmal nach einem serialisierten Refresh wiederholt. Dadurch lösen parallele `401`-Antworten nicht mehrere Refresh-Aufrufe mit demselben Einmal-Token aus.
...
Der Desktop-Bootstrap stellt der vertrauenswürdigen lokalen Oberfläche eine begrenzte `PokeAccount`-Fassade für Status, Registrierung, Login, authentifizierten Passwortwechsel, Passwort-Wiederherstellung, Sitzungswiederherstellung, Logout und Geräteverwaltung bereit. `PokeCatalog` löst validierte öffentliche Provider-Referenzen in stabile globale Karten-UUIDs auf und lädt Karten für eingehende Sync-Änderungen. Die zusätzliche `PokeSyncTransport`-Fassade überträgt validierte Operations-Batches und Change-Cursor über denselben nativen Cloud-Service. Alle drei Brücken teilen dadurch exakt eine Access-/Refresh-Sitzung; Tokenmaterial bleibt im nativen Prozess beziehungsweise im DPAPI-Speicher. JavaScript erhält nur die eigene stabile Benutzer-ID zur lokalen Cache-Partitionierung, Origin, streng validierte aktive Geräte, Ablaufzeit, strukturierte Katalog-/Sync-Nutzdaten und Fehler, niemals Tokens. Ein erfolgreicher Passwortwechsel lässt die aktuelle Sitzung aktiv und widerruft serverseitig alle anderen Geräte; beide Passwortwerte werden nach dem einmaligen nativen HTTPS-Aufruf verworfen und nie zurückgegeben. Fremde aktive Sitzungen lassen sich außerdem einzeln oder gesammelt widerrufen; die aktuelle Sitzung bleibt dem expliziten Logout vorbehalten. Reset-Code und neues Passwort werden nur an den einmaligen nativen HTTPS-Aufruf übergeben und weder im Browser-Speicher noch in Bridge-Antworten abgelegt; nach erfolgreicher Bestätigung löscht der Client seine lokale Sitzung. Die Benutzer-ID ist reine Serverausgabe und wird nicht als Besitzfilter an Endpunkte zurückgesendet. Pro Fassade sind höchstens acht parallele Anforderungen mit 30 Sekunden Zeitlimit zulässig. Die gemeinsame Systemseite bietet nun die sichtbaren Kontoaktionen, manuellen Sync, Cloud-Bestandsstatus und die ausdrücklich bestätigte lokale Datenübernahme.
...
Der Queue-Inhalt enthält keine Auth-Tokens und ist nicht über die generische `PokeNative.loadLocalData`-Fassade adressierbar. Refresh-Tokens bleiben separat DPAPI-geschützt. Der Queue-Snapshot selbst stützt sich derzeit auf die Zugriffskontrolle des angemeldeten Windows-Benutzerprofils; eine SQLite-basierte Ablage ist vor sehr großen Offline-Sammlungen zu evaluieren. `account-migration-core.js` versieht jede Legacy-Position vor dem Upload mit deterministischen, kontogebundenen UUIDv8-IDs, löst Provider-Karten auf, dedupliziert gegen gezogene Cloud-Holdings und schreibt höchstens 100 Operationen pro atomarem Queue-Übergang. Abgelehnte oder unvollständige Positionen bleiben sichtbar lokal erhalten.

Der gemeinsame `collection-cloud-driver.js` lädt gezogene Holdings über `PokeCatalog`, materialisiert sie auch auf einem zweiten Gerät und verknüpft bestehende Rich-Metadaten ohne kontofremde Wiederverwendung. Bereits verknüpfte lokale Mengenänderungen werden als atomare Deltas, Notizen, Sprache, Variante und Zustand als optimistic-concurrency-Updates vorgemerkt. Bei laufender Queue, unvollständigem Pull, beschädigtem Holding oder Kontowechsel wird kein veralteter Snapshot geschrieben. Neue post-migration Scans werden über einen persistenten Create-Outbox-Marker katalogisiert, gegen vorhandene Holdings dedupliziert und mit deterministischen IDs wiederanlauffähig queued. Der Marker verschwindet erst nach bestätigtem Pull; Konflikte und Ablehnungen behalten die lokale Karte samt Menge sichtbar.
