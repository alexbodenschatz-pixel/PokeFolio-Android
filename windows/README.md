...
Der Desktop-Bootstrap stellt der vertrauenswürdigen lokalen Oberfläche eine begrenzte `PokeAccount`-Fassade für Status, Registrierung, Login, Wiederherstellung und Logout bereit. `PokeCatalog` löst validierte öffentliche Provider-Referenzen in stabile globale Karten-UUIDs auf und lädt Karten für eingehende Sync-Änderungen. Die zusätzliche `PokeSyncTransport`-Fassade überträgt validierte Operations-Batches und Change-Cursor über denselben nativen Cloud-Service. Alle drei Brücken teilen dadurch exakt eine Access-/Refresh-Sitzung; Tokenmaterial bleibt im nativen Prozess beziehungsweise im DPAPI-Speicher. JavaScript erhält nur die eigene stabile Benutzer-ID zur lokalen Cache-Partitionierung, Origin, Gerät, Ablaufzeit, strukturierte Katalog-/Sync-Nutzdaten und Fehler, niemals Tokens. Die Benutzer-ID ist reine Serverausgabe und wird nicht als Besitzfilter an Endpunkte zurückgesendet. Pro Fassade sind höchstens acht parallele Anforderungen mit 30 Sekunden Zeitlimit zulässig. Der persistente Queue-Treiber ist vorhanden; eine sichtbare Account-Oberfläche und die nicht-destruktive Bindung der bestehenden lokalen Sammlung an Server-Entities folgen getrennt.
...
Der Queue-Inhalt enthält keine Auth-Tokens und ist nicht über die generische `PokeNative.loadLocalData`-Fassade adressierbar. Refresh-Tokens bleiben separat DPAPI-geschützt. Der Queue-Snapshot selbst stützt sich derzeit auf die Zugriffskontrolle des angemeldeten Windows-Benutzerprofils; eine SQLite-basierte Ablage ist vor sehr großen Offline-Sammlungen zu evaluieren. Die bestehende sichtbare `pf_collection` ist noch nicht an Server-Entities gebunden. Bis diese nicht-destruktive Migration implementiert ist, ist der Treiber Infrastruktur und keine vollständige Cloud-Sammlungsoberfläche.
