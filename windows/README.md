...
Der Queue-Inhalt enthält keine Auth-Tokens und ist nicht über die generische `PokeNative.loadLocalData`-Fassade adressierbar. Refresh-Tokens bleiben separat DPAPI-geschützt. Der Queue-Snapshot selbst stützt sich derzeit auf die Zugriffskontrolle des angemeldeten Windows-Benutzerprofils; eine SQLite-basierte Ablage ist vor sehr großen Offline-Sammlungen zu evaluieren. `account-migration-core.js` versieht jede Legacy-Position vor dem Upload mit deterministischen, kontogebundenen UUIDv8-IDs, löst Provider-Karten auf, dedupliziert gegen gezogene Cloud-Holdings und schreibt höchstens 100 Operationen pro atomarem Queue-Übergang. Abgelehnte oder unvollständige Positionen bleiben sichtbar lokal erhalten. Die vollständige Hydrierung der Cloud-Sammlung auf einem zweiten Gerät und die automatische Übersetzung jeder späteren lokalen Bearbeitung in Sync-Operationen sind noch offen.
