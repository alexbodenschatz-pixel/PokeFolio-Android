...
Der Desktop-Bootstrap stellt der vertrauenswürdigen lokalen Oberfläche eine begrenzte `PokeAccount`-Fassade für Status, Registrierung, Login, Wiederherstellung und Logout bereit. Die zusätzliche `PokeSyncTransport`-Fassade überträgt validierte Operations-Batches und Change-Cursor über denselben nativen Cloud-Service. Beide Brücken teilen dadurch exakt eine Access-/Refresh-Sitzung; Tokenmaterial bleibt im nativen Prozess beziehungsweise im DPAPI-Speicher. JavaScript erhält nur die eigene stabile Benutzer-ID zur lokalen Cache-Partitionierung, Origin, Gerät, Ablaufzeit, strukturierte Sync-Nutzdaten und Fehler, niemals Tokens. Die Benutzer-ID ist reine Serverausgabe und wird nicht als Besitzfilter an Endpunkte zurückgesendet. Pro Fassade sind höchstens acht parallele Anforderungen mit 30 Sekunden Zeitlimit zulässig. Eine sichtbare Account-Oberfläche und der persistente Queue-Treiber zwischen `sync-core.js`, lokaler Sammlung und diesem Transport sind noch offen und folgen getrennt.
